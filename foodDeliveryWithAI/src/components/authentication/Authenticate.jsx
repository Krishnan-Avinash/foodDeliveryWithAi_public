...
const Authenticate = () => {
  ...
  async function fetchCart() {
    const response = await axios.get(
      `http://localhost:6374/API/aiFoodDelivery/cart/getCart?email=${userFromRedux.email}`
    );
  ...
  async function handleCallbackResponse(response) {
    ...
    try {
      const res = await axios.get(
        `http://localhost:6374/API/aiFoodDelivery/user/getUser?email=${userObject.email}`
      );
      ...
      if (error.response.data.success == false) {
        ...
        try {
          const registrationResponse = await axios.post(
            "http://localhost:6374/API/aiFoodDelivery/user/newUser",
            newUser
          );
